...
def main():
    ...
    df = pd.DataFrame(columns=['Date','Time','Focus','Emotion','Head','Pointing'])
    ...
    def videoDisplayFunc(video_display):
        if video_display:
    ...
    while start == 0:
        ...
        # Drawing part
        wavingVal = 'not waving'
        movingVal = 'not moving'

        # Scoring part
        df = score(eyeFocusVal,emotionVal,headVal,pointingVal,wavingVal,movingVal,next,df)
