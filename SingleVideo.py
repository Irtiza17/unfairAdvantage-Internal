...
camera = "video" #Camera Values can be "cam","othercam","video","oakD", depending on income stream source.
multivideo = False
TestingFolder = 'Focussed on Screen'
VideoName = "1057653187-preview.mp4"
dataset_path = f'Testing Videos/{TestingFolder}/{VideoName}'
...
def main():
    ...
    def calc_landmark_list(image, landmarks,ROI=False,hand = False):
        ...
        if ROI == False:
            ...
            for i in ROI:
                ...
                landmark_y = min(int(landmark.y * image_height), image_height - 1)
                landmark_point.append([landmark_x, landmark_y])
                cv.circle(image,(landmark_x,landmark_y),2,(0,255,0),-1)
    ...
    while start == 0:
        ...
        if key2 == ord('s'):  # Save the frame to file
            print(VideoName)
            count+=1
            cv.imwrite(f"Model Testing/{TestingFolder}/{VideoName}-{count}.jpg" , image)
